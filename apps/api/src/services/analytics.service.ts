/**
 * analytics.service.ts — student Test Series analytics aggregate.
 *
 * Subject key: courses.subject → courses.title → "General"
 */
import type {
...
export async function getStudentTestAnalytics(
  userId: string,
): Promise<StudentTestAnalytics> {
  const supabase = getSupabaseAdmin();

  const { data: attemptsRaw, error: attemptsError } = await supabase
    .from('test_attempts')
    .select(
      'id, test_id, obtained_marks, percentage, is_passed, submitted_at',
    )
    .eq('user_id', userId)
  ...
  if (attemptsError) {
    throw new AppError(500, 'ANALYTICS_FETCH_FAILED', attemptsError.message);
  }
  ...
  if (testsError) {
    throw new AppError(500, 'ANALYTICS_FETCH_FAILED', testsError.message);
  }
  ...
  if (courseIds.length > 0) {
    const { data: coursesRaw, error: coursesError } = await supabase
      ...
      .in('id', courseIds);
    if (coursesError) {
      throw new AppError(500, 'ANALYTICS_FETCH_FAILED', coursesError.message);
    }
  ...
  const passPercentage = (passed / totalAttempts) * 100;
  ...
  const strong_subjects = subjectStats.slice(0, 3);
  const weak_subjects = [...subjectStats]
  ...
  // Score over time (chronological, last 14 days with data)
  ...
  const score_over_time = Array.from(byDay.entries())
    .map(([date, b]) => ({
      date,
  ...
  return {
    summary: {
      ...
      pass_percentage: round1(passPercentage),
    },
    strong_subjects,
    weak_subjects,
    recent_activity,
...
function emptyAnalytics(): StudentTestAnalytics {
  return {
    summary: {
      average_score: 0,
      total_tests: 0,
